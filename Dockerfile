...
RUN comfy node install --exit-on-fail rgthree-comfy@1.0.2512112053 --mode remote || (echo "WARN: rgthree-comfy@1.0.2512112053 unavailable in registry, falling back to latest" >&2 && comfy node install --exit-on-fail rgthree-comfy --mode remote)
...
RUN BACKOFFS="10 20 30 60 90" && for i in 1 2 3 4 5; do HF_TOKEN=$HF_TOKEN comfy model download --url 'https://huggingface.co/catlover1937/Moody-Real-Mix/resolve/main/moody-real-V7Global_00004__clean.safetensors?download=true' --relative-path models/diffusion_models --filename 'ComfyUI/moody-real-v7.6_00001_.safetensors' && break; if [ $i -eq 5 ]; then echo "model-download failed after 5 attempts" >&2; exit 1; fi; SLEEP=$(echo $BACKOFFS | cut -d ' ' -f $i) && echo "model-download attempt $i failed; retrying in $SLEEP seconds" >&2; sleep $SLEEP; done
